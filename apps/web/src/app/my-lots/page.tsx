  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="max-w-md mx-auto bg-slate-900/90 p-8 rounded-3xl border border-amber-500/30 shadow-2xl text-center space-y-4 my-8">
        ...
        <h2 className="text-xl font-black text-white">Farmer Login Required</h2>
        <p className="text-xs text-slate-400">Please sign in to view your published crop lots and received bids.</p>
        <div className="pt-2">
          <Link
            href="/login"
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-amber-500/20 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Manage harvest listings, review incoming offers, and finalize deals</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/80 p-10 rounded-3xl border border-amber-500/20 shadow-sm text-center space-y-4 max-w-lg mx-auto">
          <div className="w-16 h-16 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-3xl flex items-center justify-center mx-auto">
            <Package className="w-8 h-8" />
          </div>
          <div className="space-y-1">
            <h2 className="text-lg font-black text-white">No Crop Lots Published Yet</h2>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs bg-slate-950 p-3 rounded-2xl border border-amber-500/20">
                    <div>
                      ...
                      <span className="font-black text-amber-300">₹{lot.expectedPrice}/Qtl</span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-bold block text-[10px]">Quality Grade</span>
                      <span className="font-black text-white">{lot.qualityGrade || 'GRADE_A'}</span>
                    </div>
                  </div>
