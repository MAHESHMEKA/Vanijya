...
export default function CreateLotPage() {
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="max-w-md mx-auto bg-slate-900/90 p-8 rounded-3xl border border-amber-500/30 shadow-2xl text-center space-y-4 my-8">
        ...
        <h2 className="text-xl font-black text-white">Sign In to Publish Produce</h2>
        <p className="text-xs text-slate-400">Please sign in with your farmer account to list crop lots for commercial buyers.</p>
        <div className="pt-2">
          <Link
            href="/login"
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5 animate-in fade-in duration-300">
      <Link href="/dashboard" className="inline-flex items-center gap-1 text-xs text-amber-400 font-bold hover:underline">
        <ArrowLeft className="w-3.5 h-3.5" /> Back to Dashboard
      </Link>

      <div className="bg-slate-900/90 p-6 md:p-8 rounded-3xl border border-amber-500/30 shadow-2xl space-y-6">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400">Direct Farm-Gate Listing</span>
          </div>
          <h1 className="text-2xl font-black text-white tracking-tight">{t.createLotTitle}</h1>
          <p className="text-xs text-slate-400">{t.createLotSubtitle}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Commodity Selection */}
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">{t.lotCropLabel}</label>
            <select
              ...
              className="w-full px-3.5 py-2.5 bg-slate-950 border border-amber-500/30 rounded-xl text-xs font-bold text-white focus:bg-slate-900 focus:ring-2 focus:ring-amber-400 focus:outline-none"
            >
              {crops.map((crop) => (
                <option key={crop.id} value={crop.name} className="bg-slate-900 text-white">
                  {crop.name}
                </option>
              ))}
            </select>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1.5">{t.lotQtyLabel}</label>
              <input
                type="number"
                required
                min="1"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1.5">{t.lotPriceLabel}</label>
              <input
                type="number"
                required
                min="1"
          ...
          {/* Produce Grade */}
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">{t.lotGradeLabel}</label>
            <select
              value={qualityGrade}
              onChange={(e) => setQualityGrade(e.target.value)}
              className="w-full px-3.5 py-2.5 bg-slate-950 border border-amber-500/30 rounded-xl text-xs font-bold text-white focus:bg-slate-900 focus:ring-2 focus:ring-amber-400 focus:outline-none"
            >
              <option value="GRADE_A" className="bg-slate-900 text-white">Grade A (Premium Export Quality)</option>
              <option value="GRADE_B" className="bg-slate-900 text-white">Grade B (Standard Market Quality)</option>
              <option value="GRADE_C" className="bg-slate-900 text-white">Grade C (Fair Average Quality)</option>
            </select>
          </div>

          {/* Location */}
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">{t.lotLocationLabel}</label>
            <input
              type="text"
              required
              value={location}
